...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # 修改为前半部分和后半部分分割（替代奇偶分割）
        seq_len = input_ids.shape[-1]
        half_len = seq_len // 2  # 计算前半部分长度
        x1_ids = input_ids[..., :half_len]  # 前半部分
        x2_ids = input_ids[..., half_len:]  # 后半部分
        
        # 对应的attention mask也按前半后半分割
        x1_mask = attention_mask[..., :half_len] if attention_mask is not None else None
        x2_mask = attention_mask[..., half_len:] if attention_mask is not None else None
