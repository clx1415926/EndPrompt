...
# 修改LlamaYaRNScaledRotaryEmbedding类以支持动态更新
...
class LlamaYaRNScaledRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, scale=1, original_max_position_embeddings=2048, 
        ...
        # 初始化时根据是否微调决定是否计算初始inv_freq
        if finetuned:
            self.yarn(device)  # 使用初始scale计算
        else:
    ...
    def forward(self, x, seq_len=None):
        # 当序列长度超过缓存时，仅延长缓存，使用初始化时确定的 inv_freq
        if seq_len > self.max_seq_len_cached:
            self.max_seq_len_cached = seq_len
            
            # 直接使用现有的 self.inv_freq 计算新的位置编码
            t = torch.arange(self.max_seq_len_cached, device=x.device, dtype=self.inv_freq.dtype)
            freqs = torch.einsum("i,j->ij", t, self.inv_freq)
            emb = torch.cat((freqs, freqs), dim=-1).to(x.device)
    ...
    def _dynamic_update_yarn(self, scale, device):
        """动态更新inv_freq和mscale的内部方法"""
        # 计算频率和掩码（复用原yarn逻辑但接受动态scale参数）
        pos_freqs = self.base ** (torch.arange(0, self.dim, 2).float().to(device) / self.dim)
        inv_freq_extrapolation = 1.0 / pos_freqs
...
# 同时修改动态版本的LlamaDynamicYaRNScaledRotaryEmbedding（如果存在）
class LlamaDynamicYaRNScaledRotaryEmbedding(LlamaYaRNScaledRotaryEmbedding):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, original_max_position_embeddings=2048, 
                 extrapolation_factor=1, attn_factor=1, beta_fast=32, beta_slow=1, finetuned=False, device=None):
        # 动态版本不需要初始scale，使用original_max_position_embeddings作为基准
        super().__init__(dim, max_position_embeddings, base, scale=1.0,  # scale参数仅为兼容，实际动态计算
                         original_max_position_embeddings=original_max_position_embeddings,
                         extrapolation_factor=extrapolation_factor, attn_factor=attn_factor,
                         beta_fast=beta_fast, beta_slow=beta_slow, finetuned=finetuned, device=device)

    # 复用父类的forward和_dynamic_update_yarn方法，无需额外实现
...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
            ...
            elif scaling_type == "vanilla_ntk": # <-- (保留了这个，以防万一)
                ...
                self.rotary_emb = LlamaYaRNScaledRotaryEmbedding(
                    self.head_dim, max_position_embeddings=self.max_position_embeddings, scale=scaling_factor, original_max_position_embeddings=original_max_position_embeddings, finetuned=True
                )
    ...
    def forward(
        ...
        padding_mask: Optional[torch.LongTensor] = None, # <-- (新增)
